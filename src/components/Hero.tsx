...
import { HeroSection } from '@/lib/types/ContentType'
function Hero({data}: {data: HeroSection}) {
  return (
    <div className='mt-50 max-sm:mt-20  max-sm:p-5 lg:pl-50 sm:p-10 menu relative max-w-[1000px]'>
        <div>
            <div className={`${FiraCode.className} text-sm text-[#9893E2] max-sm:text-xs`}><span>{data.hi_my_name_is}</span></div>
        </div>
        ...
        <div className={`${Inter.className} mt-3 `}>
            <h2 className='text-4xl text-[#bab9ce6b] font-semibold max-lg:text-4xl max-sm:text-2xl max-md:text-4xl'>{data.i_do_this}</h2>
            <p className={`text-[#C2C2C2] md:w-1/2 mt-2 text-sm max-w-[800px] max-sm:text-xs`}>
            {data.i_am_right_now}
            </p>
        </div>
        <div className='mt-10'>
            <VioletButton title={data.download_cv} Icon={ArrowDownTrayIcon} link='cv'/>
        </div>
